...
/* Angle ICON */
const AngleIcon: new (options: AngleMarkerIconOptions) => L.Icon = L.Icon.extend({
    // default value
    options: {
        angle: 0,
    ...
    draw: function (ctx: any, w: number, h: number) {
     ...
     * @param h height
     */
    drwaImage: function (ctx: any, img: any, course: any, w: number, h: number) {
        console.log(img.src)
        img.onload = function () {
            //平移坐标原点
            ctx.translate(40, 30);
     ...
     * @version   2016-08-19
     */
    getAngle: function (A: L.LatLng, B: L.LatLng) {
        var angle = null;
        var latA = A.lat;
        var lonA = A.lng;
        var latB = B.lat;
        ...
        // 注意经度或者纬度都不相等 (Longitude and latitude are not equal)
            ...
            var y1 = A.lng * Math.pow(10, 12);
            var y2 = B.lng * Math.pow(10, 12);
...
type AngleMarkerOptions = L.MarkerOptions & {
    label: string,
    labelColor: string,
    img: string,
    labelFlag: true
...
export declare class AngleMarker extends L.Marker {
    getAngle(a: L.LatLng, b: L.LatLng): number;
...
const AngleMarkerCls: new (pos: L.LatLng, options: AngleMarkerOptions) => AngleMarker = L.Marker.extend({
    getAngle: function (A: L.LatLng, B: L.LatLng) {
...
export const angleMarker = function (pos: L.LatLng, options: AngleMarkerOptions) {
    const img = new Image();
    img.src = options.img;

    const icon = new AngleIcon({
        ciFlag: options.labelFlag,
        label: options.labelFlag ? options.label : undefined,
        textColor: options.labelFlag ? options.labelColor : undefined,
        img: img
    });

    (options as any).icon = icon;

    return new AngleMarkerCls(pos, options);
};